...
class BILEModule:
    """Online BILE encoder and dynamics model for fixed-set exploration.

    The encoder maps the current RL observation to a compact behavioral metric
    space. The dynamics model predicts the next observation from the current
    observation and the joint set/operation action.
    """
    ...
    def _create_encoder(self):
        return tf.keras.Sequential(
            [
                ...
                Dense(self.dense_units, activation="relu"),
                Dense(self.latent_dim, activation="linear"),
            ]
        )
    ...
    def _create_dynamics(self):
        ...
        return tf.keras.Sequential(
            [
                Input((self.state_dim + action_dim,)),
                Dense(self.dense_units, activation="relu"),
                Dense(self.dense_units, activation="relu"),
